...
def process_image(image_path, model, threshold=0.5):  
    try:
        ...
        # Добавляем валидацию размеров изображения
        if image.shape[0] < 64 or image.shape[1] < 64:
            raise ValueError("Image dimensions too small")

        # Оптимизируем предобработку
        image = cv2.resize(image, (512, 512))
        image = adjust_gamma(image, gamma=0.8)
        
        # Применяем CLAHE с адаптивными параметрами
        clahe = cv2.createCLAHE(
            clipLimit=2.0,
            tileGridSize=(8,8) if image.shape[0] <= 256 else (16,16)
        )
...
def main():
    ...
    test_image = "Anon_Liver/Abdomen - 3928/ART_2/IM-0013-0093.dcm"
